...
export function MobileNav({ isLoggedIn, isAdmin, showNav = true }: MobileNavProps) {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden safe-area-pb">
            ...
            <div className="relative mx-auto flex h-14 w-[min(92vw,420px)] items-center justify-between gap-1 rounded-full border border-primary/15 bg-background/85 px-2 shadow-lg shadow-primary/5 backdrop-blur-xl">
                {navItems.map((item) => (
                    <Link
                        key={item.href}
                        href={item.href}
                        className={cn(
                            "relative flex flex-1 flex-col items-center justify-center gap-0.5 rounded-full py-1.5 text-[11px] font-medium transition-all duration-200 cursor-pointer",
                            item.active
